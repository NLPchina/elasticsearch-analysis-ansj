...
/**
 * 分词的cat Created by zhangqinghua on 16/2/2.
 */
		...
		if (texts == null || texts.length == 0) {
			...
			client.admin().indices().analyze(analyzeRequest, new RestResponseListener<AnalyzeResponse>(channel) {
				public RestResponse buildResponse(final AnalyzeResponse analyzeResponse) throws Exception {
					return ChineseRestTable.reponse(channel,
							"err args exapme : /_cat/analyze?text=中国&analyzer=index_ansj, other params: [field,tokenizer,token_filters,char_filters]");
				}
			});
		} else {
			AnalyzeRequest analyzeRequest = new AnalyzeRequest(request.param("index"));
