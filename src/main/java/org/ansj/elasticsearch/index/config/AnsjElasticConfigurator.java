...
public class AnsjElasticConfigurator {
    ...
    public static void init(Environment env) {
        ...
        if (Files.exists(configFilePath)) {
            try {
                ...
                logger.info("load ansj config: {}", configFilePath);
            } catch (IOException e) {
    ...
    private static void initRedis(final Settings settings, Environment environment) {
    ...
    private static void initConfig(Settings settings, Environment environment) {
        ...
        // 是否开启人名识别
        MyStaticValue.isNameRecognition = settings.getAsBoolean("enable_name_recognition", MyStaticValue.isNameRecognition);

        // 是否开启数字识别
        MyStaticValue.isNumRecognition = settings.getAsBoolean("enable_num_recognition", MyStaticValue.isNumRecognition);

        // 是否数字和量词合并
        MyStaticValue.isQuantifierRecognition = settings.getAsBoolean("enable_quantifier_recognition", MyStaticValue.isQuantifierRecognition);
        ...
        try {
            String jarPath = java.net.URLDecoder.decode(
                    ...
                    "UTF-8");
            String defaultPath = new File(new File(jarPath).getParent(), "default.dic").getAbsolutePath();
    ...
    private static void loadRedisLib(Settings settings, Environment environment) {
