...
import org.elasticsearch.index.analysis.AnsjStaticValue;

import static org.elasticsearch.index.analysis.AnsjStaticValue.logger;
...
public class UserDefineLibrary {
    ...
    private static void initAmbiguityLibrary() {
        ...
        if (StringUtil.isBlank(ambiguityLibrary)) {
            logger.warn("init ambiguity  waring :" + ambiguityLibrary
        ...
        File file = new File(AnsjStaticValue.environment.configFile(),ambiguityLibrary);
        if (file.isFile() && file.canRead()) {
            try {
                ambiguityForest = Library.makeForest(file.getPath());
            } catch (Exception e) {
                // TODO Auto-generated catch block
                logger.warn("init ambiguity  error :" + ambiguityLibrary
                                   + " because : not find that file or can not to read !");
                e.printStackTrace();
            }
            logger.info("init ambiguityLibrary ok! Path is :"+file.getPath());
        } else {
            logger.warn("init ambiguity  waring :" + ambiguityLibrary
                               + " because : not find that file or can not to read !");
        }
    ...
    public static void loadFile(Forest forest, File file) {
        // TODO Auto-generated method stub
        if (!file.canRead()) {
            logger.warn("file in path " + file.getAbsolutePath() + " can not to read!");
            return;
        }
        String temp = null;
        ...
        try {
            ...
            logger.info("init user userLibrary ok path is : " + file.getAbsolutePath());
    ...
    public static void loadLibrary(Forest forest, String path) {
        ...
        File file = null;
        if (path != null) {
            file = new File(AnsjStaticValue.environment.configFile(),path);
            if (!file.canRead() || file.isHidden()) {
                logger.warn("init userLibrary  waring :" + path
                                   + " because : not find that file or can not to read !");
                return;
            }
            if (file.isFile()) {
                ...
                logger.warn("init user library  error :" + path
